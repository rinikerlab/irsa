...
class Spectrum:
    def __init__(self,exp,freq,E,T,lower_bound,higher_bound,w1,w2,resolution_exp,Dipol,out,peaks=0,mu=1):
        ...
        self.E = E
        self.T = T
##ORIGINALL, MU WAS NOT IN THIS PART
        self.mu = mu
        ...
        try:
            print(len(peaks))
            self.exp_peaks=peaks
            val = (self.exp_peaks[:,0] > self.u) & (self.exp_peaks[:,0]< self.h)
            self.exp_peaks = self.exp_peaks[val]
            self.exp_peaks[:,1] /= np.max(self.exp_peaks[:,1])
        except:
            self.exp_peaks()
            print('auto set peaks')
            pass
        print("Object spectrum successfully created")
    def compute_Z(self): 
        """Compute Z in N_i/N = \sum_i e^(E_i/(RT))/Z"""
        self.E = self.E - np.min(self.E)
        self.Z_array = np.exp(-self.E/(self.T*8.314*10**(-3)))
        self.Z = np.sum(self.Z_array,axis=-1)
    def Boltzmann_weight(self):
        ...
        self.spectrum_boltzmann[:,0] = x_value*self.mu ##x_axis 0..2000
        if(len(self.E)>1):
            self.spectrum_boltzmann[:,1] = np.sum(self.spectrum[:,:,1]*(self.Z_array/self.Z)[:,np.newaxis],axis=0)
        else:
    ...
    def plot_unshifted(self):
        ...
        np.savetxt(self.out+"unshifted.txt",self.spectrum_boltzmann)
        np.savetxt(self.out+"exp.txt",self.exp)
        py.plot()
        ...
        np.savetxt(self.out+'theo_peaks.txt',self.theo_peaks)
        #py.plot(self.exp_peaks[:,0],self.exp_peaks[:,1],"x",color="black")
        py.xlim(self.h,self.u)
        py.xlabel("$\\tilde{\\nu}$ / cm$^{-1}$")
        py.ylabel("$I$ / a.u.")
        py.ylim(0,1.02)
        py.savefig(self.out+"unshifted.png",dpi=800)
        py.cla()
        py.clf()
        py.close()
    ...
    def plot(self):
        ...
        np.savetxt(self.out+"aligned.txt",self.spectrum_bc)
        np.savetxt(self.out+"exp.txt",self.exp)
        py.cla()
        py.clf()
        py.close()
    def integrate(self,number_of_bins=150):
        ...
        sp = spearmanr(integ_exp[:,1],integ_theo[:,1])[0]
        return p,sp
