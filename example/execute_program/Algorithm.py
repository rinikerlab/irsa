...
##ORIGINALLY, THE SHIFTING WAS ONLY PERFORMED HERE. I CHANGED THIS TO THE CREATION OF THE SPECTRA
        self.mu = 1
##
    def Diagonal_IR(self,freq_i,inten_i,exp_freq_j,exp_inten_j,bond_l,bond_h,n,m):
        ...
        if(abs(exp_freq_j-freq_i*1)<self.cutoff):
    ...
    def Backtrace_IR(self,p_mat,al_mat,n,m,freq_i,inten_i,exp_freq_j,bond_l,bond_h): #n theoretical, m experimental
        ...
        while(True):
            ...
            if(p_mat[n,m]=="D"):
                ...
                tmp.append(exp_freq_j[m-1])
                gain_1 = al_mat[n-1,m-1]
                tmp.append(gain_1)
                region_of_interest.append(tmp)
                n=n-1
                m=m-1
            elif(p_mat[n,m]=="V"):
        ...
        return new_freq,new_inten,old_freq, region_of_interest
    ...
    def Needleman_IR(self):
        ...
        freq,inten,old_freq,region_of_interest = self.Backtrace_IR(p_mat,al_mat,n,m,freq,inten,exp_freq,bond_l,bond_h)
        returnvalue = al_mat[n-1,m-1]#/(n) ##NORMALIZE VALUE BY NUMBER OF THEORETICAL PEAKS. MIGHT BE CHANGED FOR MORE
                                          ##SENSIBLE VALUES
        return returnvalue, old_freq, freq, inten,np.asarray(region_of_interest)
