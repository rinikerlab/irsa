...
parser.add_argument('-d0','--dummy0',required=False,default=0,type=int)
parser.add_argument('-d1','--dummy1',required=False,default=0,type=int)
parser.add_argument('-o','--out',required=False,default="",type=str)
parser.add_argument('-p','--peaks',required=False,default="",type=str)
...
class Main:
    ...
    def run(self):
            ...
            try:
                p = np.loadtxt(peaks,usecols=(0,1,)) #peaks
                idx = np.argsort(p[:,0])
                p = p[idx]
            except:
                p = 0
                print('no peaks set')
                pass
            self.Spectrum = Spectrum.Spectrum(self.exp,self.freq,self.E,T=T,lower_bound=u,higher_bound=h,w1=width1,w2=width2,resolution_exp=resolution,Dipol=dipol,out=out,peaks=p,mu=mu)
            Alg = Algorithm.Algorithm(self.Spectrum,mu=mu,sigma_0=sigma0,sigma_1=sigma1,cutoff=cutoff,dummy_0=dummy_0,dummy_1=dummy_1)
            returnvalue, freq_old, freq_new, inten_new, region_of_interest = Alg.Needleman_IR()
            print(-returnvalue)
            self.Spectrum.set_new_freq(freq_old,freq_new,inten_new)
            self.Spectrum.plot_unshifted()
            self.Spectrum.create_shifted()
            self.Spectrum.plot()
            p,sp = self.Spectrum.integrate()
            self.Reader.write(out,p,sp,returnvalue,region_of_interest,args)
            return 1
        #except:
        #    return 1
program = Main()
